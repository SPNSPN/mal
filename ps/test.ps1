...
#echeck "(to-queue 33)" $erroid["Type"] "cannot cast 33 to QueuT.";
...
check "(empty '(1))" $nil;
check "(empty [1])" $nil;
#check "(empty (pushqueue (queue) 1))" $nil;
check "(empty `"1`")" $nil;
...
#check "(type (queue))" (new-object symb "<queu>");
